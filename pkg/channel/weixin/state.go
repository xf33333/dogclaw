...
func buildWeixinSyncBufPath(cfg config.WeixinSettings) string {
	return filepath.Join(getDogclawHome(), "channels", "weixin", "sync", genWeixinAccountKey(cfg)+".json")
}

func buildWeixinContextTokensPath(cfg config.WeixinSettings) string {
	return filepath.Join(getDogclawHome(), "channels", "weixin", "context-tokens", genWeixinAccountKey(cfg)+".json")
}
...
func saveGetUpdatesBuf(path, cursor string) error {
	...
	// Use simple write for now or implement atomic write if needed
...
func (c *WeixinChannel) cdnBaseURL() string {
	if base := strings.TrimSpace(c.config.BaseURL); base != "" {
		// Note: Using BaseURL as fallback for CDN if not explicitly separated in settings
...
func (c *WeixinChannel) pauseSession(operation string, ret, errcode int, errmsg string) time.Duration {
	...
	logger.Errorf("[weixin] Session expired during %s; pausing channel. code=%d/%d msg=%s. until=%s",
		operation, ret, errcode, errmsg, c.pauseUntil.Format(time.RFC3339))
	return remaining
}
