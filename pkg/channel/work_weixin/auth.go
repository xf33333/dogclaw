...
func PerformLoginInteractive(
	...
	logger.Info("[weixin] Requesting Weixin QR code...")
	...
	for {
		...
		case <-pollTicker.C:
			...
			case "confirmed":
				...
				logger.Infof("[weixin] Login successful for account: %s", statusResp.IlinkBotID)
			...
			case "scaned_but_redirect":
				if statusResp.RedirectHost == "" {
					logger.Warn("[weixin] scaned_but_redirect received without redirect_host; continuing on current host")
					continue
				}
				nextBaseURL := "https://" + statusResp.RedirectHost + "/"
				nextAPI, nextErr := NewApiClient(nextBaseURL, "", opts.Proxy)
				if nextErr != nil {
					logger.Warnf("[weixin] Failed to switch QR polling host to %s: %v", statusResp.RedirectHost, nextErr)
					continue
				}
				pollAPI = nextAPI
				logger.Infof("[weixin] Switched QR polling host to %s", statusResp.RedirectHost)
			case "expired":
				return "", "", "", "", fmt.Errorf("qrcode expired, please try again")
			default:
				logger.Warnf("[weixin] Unknown QR code status: %s", statusResp.Status)
			}
		}
	}
}
