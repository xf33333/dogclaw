...
func getExecutablePath() string {
	...
	// 尝试找到 dogclaw 可执行文件
	// 1. 首先检查与 DogClawUI 同一目录下的 dogclaw.exe
	if exe, err := os.Executable(); err == nil {
		exeDir := filepath.Dir(exe)
		dogclawPath := filepath.Join(exeDir, "dogclaw.exe")
	...
	// 2. 检查 PATH
	if path, err := exec.LookPath("dogclaw.exe"); err == nil {
	...
	if path, err := exec.LookPath("dogclaw"); err == nil {
		log.Printf("Found dogclaw in PATH (without .exe): %s", path)
		return path
	}

	// 4. 尝试构建路径
	wd, _ := os.Getwd()
	path := filepath.Join(wd, "dogclaw.exe")
...
// showAlert 显示 Windows 提示框
func showAlert(title, message string) {
	// 使用 Windows 的 msg 命令显示提示框
